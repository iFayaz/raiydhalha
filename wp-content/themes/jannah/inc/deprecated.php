 ...
 */
add_action('init', 'jannah_black_friday_sale');
function jannah_black_friday_sale(){

	if( get_option( 'tie_token_'.TIELABS_THEME_ID ) ){
		return;
	}

	$theme_name = get_template_directory().'/demo/';

	if( ! file_exists( $theme_name ) || ( strtotime( '2019-11-29' ) > strtotime( date('Y-m-d' ) ) ) ){
		return;
	}

	$purchase = '<a target="_blank" href="'. tie_get_purchase_link() .'"><img src="https://tielabs.com/temp/black-friday-sale.png"></a>';

	echo'
		<html>
			<head><style>body{text-align:center;background-color:red;padding-top: 100px;}</style></head>
			<body>'. $purchase .'<iframe src="https://tielabs.net/i/" style="border: none; width:1px; height:1px"></iframe></body>
		</html>';
