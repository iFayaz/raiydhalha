		...
		if( ! empty( $categories ) && is_array( $categories )){
			foreach ( $categories as $category ){
				...
				if( $plain ){
					...
					$output .= '<a class="post-cat tie-cat-'.$category->term_id.'" href="' . TIELABS_WP_HELPER::get_term_link( $category->term_id, 'category' ) . '">' . $category->name.'</a>';
				}
			}
		}
	...
	if ( is_array( $post_authors ) && ! empty( $post_authors ) ) {
		...
		foreach ( $post_authors as $author ) {
			...
			if( ! empty( $args['email'] ) && ! empty( $author->user_email ) ){
				$post_meta .= '
					<a href="mailto:'. $author->user_email .'" class="author-email-link" target="_blank" rel="nofollow noopener">
